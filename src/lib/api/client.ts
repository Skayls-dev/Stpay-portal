...
const API_KEY_FALLBACK = String(import.meta.env.VITE_API_KEY ?? '').trim()
...
client.interceptors.request.use(
  (config) => {
    ...
    const effectiveApiKey = apiKey || API_KEY_FALLBACK
    if (effectiveApiKey) {
...
client.interceptors.response.use(
  ...
  (error) => {
    ...
    const isPublicDemoRoute = typeof window !== 'undefined' && window.location.pathname.startsWith('/demo/')
    const isPublicAuthCall =
      requestUrl.includes('/api/admin/login')
      || requestUrl.includes('/api/merchant/login')
      || requestUrl.includes('/api/merchant/register')

    if (error?.response?.status === 401 && !isPublicAuthCall && !isApiKeyAuthFailure && !isPublicDemoRoute) {
